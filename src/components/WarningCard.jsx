  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-panel px-3 py-2 rounded-lg border border-danger/30 shadow-xl text-xs">
        <p className="font-bold text-text-primary mb-1">Month {label}</p>
        {payload.map((p, i) => (
...
export default function WarningCard() {
  ...
  const data = Array.from({ length: 12 }, (_, i) => {
    return {
      month: i + 1,
      Card: Math.round(initialDebt * Math.pow(cardRate, i + 1)),
      Loan: Math.round(initialDebt * Math.pow(loanRate, i + 1)),
    };
  });

  const finalCardCost = data[11]['Card'];
  const finalLoanCost = data[11]['Loan'];
      ...
      <div className="max-w-[90%] md:max-w-[70%] w-full">
        <div className="glass-card rounded-lg overflow-hidden border-danger/40 shadow-[0_0_15px_rgba(239,68,68,0.15)]">
          ...
          <div className="flex items-center gap-2 px-4 pt-3 pb-2 bg-danger/10 border-b border-danger/20">
            ...
            <h3 className="text-xs font-bold text-danger tracking-wide uppercase">Revolving Interest Alert</h3>
          </div>

          <div className="px-4 py-3">
            <p className="text-xs text-text-secondary leading-relaxed mb-3">
              See how a debt of <strong>R$ 1,000.00</strong> grows over 12 months paying only the minimum on the card vs. a personal loan:
            </p>
            ...
            <div className="h-36 w-full mb-2">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="Card" stroke="#ef4444" strokeWidth={2} fillOpacity={1} fill="url(#colorCard)" />
                  <Area type="monotone" dataKey="Loan" stroke="#00a884" strokeWidth={2} fillOpacity={1} fill="url(#colorLoan)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="mt-3 bg-danger/5 rounded p-2 border border-danger/10">
              <div className="flex items-start gap-2">
                ...
                <p className="text-[11px] text-text-primary">
                  By switching the debt to a personal loan at 3% per month, you would save <strong>{formatCurrency(totalSavings)}</strong> in interest over the year.
                </p>
              </div>
            </div>
          </div>
